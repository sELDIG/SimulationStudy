...
alt.x.labels[c(2, 8, 12, 15, 16, 17, 18, 19)] = c("tree length", "Yule-PDA ratio", "mean I'", "MGL prin. eigenvalue", "MGL asymmetry", "MGL peakedness", "MGL eigengap", "nLTT stat")
...
```{r}
x = barplot(t(predictabilityR2CV), beside = T, las = 1, horiz = F, ylim = c(0,1.1), ylab = "Correlation predicted observed")
...
for(i in 1:length(x)){
  if(!is.na(x2[i])){
    text(x[i],x2[i] + 0.02, modelAbbrevs$new[(i-1)%%8 + 1], srt = 90,cex= 0.8, adj = 0)    
  }
}
...
title(main = paste("Random forest cross-model predictability [R2], mean =", round(mean(CpredictabilityR2), digits = 3)),, xlab = "Model predicted to", ylab = "Model trained \n")
...
############################################################
# FLORIAN - double check that these RF models correspond to
#           the final models we present above
############################################################
